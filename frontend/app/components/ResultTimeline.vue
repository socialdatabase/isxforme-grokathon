  ...
  <div class="timeline-container">
    <div class="timeline-layout">
      ...
      <div class="timeline-feed">
        ...
        <div v-if="timelinePostsLoading" class="posts-loading">
        ...
        <!-- Posts List -->
        <template v-else>
          <div v-for="({ post, account }, index) in timelinePosts" :key="post.id" class="tweet">
            <div class="tweet-avatar">
              <img :src="account.profile_image_url ?? ''" :alt="account.username" />
            </div>
            <div class="tweet-content">
              <div class="tweet-header">
                <span class="tweet-name">{{ account.name }}</span>
                <svg v-if="account.verified" class="tweet-verified" width="16" height="16" viewBox="0 0 24 24" fill="#1d9bf0">
                ...
                <span class="tweet-handle">@{{ account.username }}</span>
                <span class="tweet-dot">·</span>
                <span class="tweet-date"><a target="_blank" :href="'https://www.x.com/i/status/' + post.id">{{ formatDate(post.created_at) }}</a></span>
              </div>
            <p class="tweet-body">{{ formatText(post.text) }}</p>
            <div v-if="post.media && post.media?.length > 0" class="tweet-media">
              <img :src="getUrl(post)" alt="Tweet media" />
            </div>
            <div class="tweet-actions">
              ...
              <button class="tweet-action retweet">
                ...
                <span>{{ formatNumber(post.retweet_count ?? 0) }}</span>
              </button>
      ...
      <div class="who-to-follow">
        <h3 class="wtf-title">
          Authority Index
          <span v-if="accountsLoading" class="wtf-loading">Loading...</span>
        </h3>
        <!-- Loading Spinner -->
        <div v-if="accountsLoading" class="wtf-loading-container">
...
const { posts, loading: l, accountsLoading , error, timelinePosts, timelinePostsLoading, accounts, keyword, inferredTopic} = storeToRefs(useDataStore())
...
function getUrl(post: ApiPost["post"]) {
  if (post.media) {
    if (post?.media[0].type === "video") {
      return post?.media[0].preview_image_url
    }
    if (post?.media[0].url) {
...
const generatePodcast = async () => {
  ...
  try {
    ...
    const keywordToUse = keyword.value || currentTopic.value || 'trending topics'
    console.log('Generating podcast for:', keywordToUse, 'with', postsContext.length, 'posts')
